...
public class FireworksActivity extends CommonAudioActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mMyHandler.sendEmptyMessage(MyHandler.MSG_SET_ANIMATION);
    ...
    private static class MyHandler extends android.os.Handler {
        ...
        static final int MSG_SET_ANIMATION = 0;

        final WeakReference<FireworksActivity> fireworksActivityWeakReference;

        MyHandler(FireworksActivity fireworksActivity) {
            fireworksActivityWeakReference = new WeakReference<>(fireworksActivity);
        }

        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            FireworksActivity fireworksActivity = fireworksActivityWeakReference.get();
            if (fireworksActivity != null) {
                if (msg.what == MSG_SET_ANIMATION) {
                    //需要加载的网页的url
                    fireworksActivity.mWebView.loadUrl("file:///android_asset/index4.html");//这里写的是assets文件夹下html文件的名称，需要带上后面的后缀名，前面的路径是安卓系统自己规定的android_asset就是表示的在assets文件夹下的意思。
                    fireworksActivity.mWebView.getSettings().setLayoutAlgorithm(WebSettings.LayoutAlgorithm.SINGLE_COLUMN);//自适应屏幕
                    fireworksActivity.mWebView.getSettings().setLoadWithOverviewMode(true);//自适应屏幕
                    fireworksActivity.mWebView.getSettings().setSupportZoom(true);
                    fireworksActivity.mWebView.getSettings().setUseWideViewPort(true);//扩大比例的缩放
                    // webView.getSettings().setBuiltInZoomControls(true);//设置是否出现缩放工具,这里我想就不出现了，影响效果
                    WebSettings settings = fireworksActivity.mWebView.getSettings();
                    // 如果访问的页面中要与Javascript交互，则webview必须设置支持Javascript
                    settings.setJavaScriptEnabled(true);
                    fireworksActivity.mWebView.setWebViewClient(new WebViewClient() {
                        public boolean shouldOverrideUrlLoading(WebView view, String url) {
                            view.loadUrl(url);
                            return true;
                        }
                    });
//                    fireworksActivity.mMyHandler.sendEmptyMessageDelayed(MyHandler.MSG_SET_ANIMATION, 5000);//5秒一循环
                }
            }
        }
    }
