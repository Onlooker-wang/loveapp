...
public class Petal {
    ...
    private void draw(Point p, Canvas canvas) {
        if (!isFinished) {
            ...
            Point t = new Point(0, this.radius).rotate(RandomUtil.degrad(this.startAngle));
            //第一个端点，为了保证圆心不会随着radius增大而变大这里固定为3
            Point v1 = new Point(0, 3).rotate(RandomUtil.degrad(this.startAngle));
            //第二个端点
            Point v2 = t.clone().rotate(RandomUtil.degrad(this.angle));
            //延长线，分别确定两个控制点
