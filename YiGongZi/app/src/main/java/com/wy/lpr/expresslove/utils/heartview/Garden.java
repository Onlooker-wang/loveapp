package com.wy.lpr.expresslove.utils.heartview;
...
public class Garden {
    ...
    public Bloom createRandomBloom(int x, int y) {
        //创建一个随机的花朵半径
        int radius = RandomUtil.randomInt(Options.minBloomRadius, Options.maxBloomRadius);
        //创建一个随机的花朵颜色
        int color = RandomUtil.randomrgba(Options.minRedColor, Options.maxRedColor, Options.minGreenColor, Options.maxGreenColor, Options.minBlueColor, Options.maxBlueColor, Options.opacity);
        //创建随机的花朵中花瓣个数
        int petalCount = RandomUtil.randomInt(Options.minPetalCount, Options.maxPetalCount);
